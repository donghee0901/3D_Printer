/*#define X_STEP_PIN         0x80 //PD7 1000 0000
  ...
  #define Z_STEP_PIN         0x08 //PB3 0000 1000
  #define Z_DIR_PIN          0x04 //PB2 0000 0100
  #define Z_STOP_PIN         0x10 //PC4 0001 0000

  #define XYEENABLE_PIN      0x40 //PD6 0100 0000
  #define ZENABLE            0x20 //PA5 0010 0000
  ...
  void Float_Comma_move()
  {
  int x_move;
  int y_move;

  x_step_plus = (xy_pos[0][0] / 0.0125);
  y_step_plus = (xy_pos[0][1] / 0.0125);

  map_x = (int)x_step_plus;
  map_y = (int)y_step_plus;

  x_step_plus -= (double)map_x;
  y_step_plus -= (double)map_y;
  ...
  void Comma_move(int x,int y)
  {
  int move_x = x;
  int move_y = y;
  char move_dir_x = DIR_STOP;
  char move_dir_y = DIR_STOP;

  if(move_x > 0){
    move_dir_x = DIR_RIGHT;
  }
  else if(move_x < 0){
    move_x *= -1;
    move_dir_x = DIR_LEFT;
  ...
  {
    x_delay = (int)(((double)400 * ((double)move_y/move_x))+0.5);
    if(x_delay < 200){
      x_delay = 400;
      y_delay = (int)(((double)400 * ((double)move_x/move_y))+0.5);
    }
  }
  ...
  void Comma_move_plus_delay(int x,int y,int base_delay)
  {
  int move_x = x;
  int move_y = y;
  char move_dir_x = DIR_STOP;
  ...
  if (on_off_x == 0) {
    ...
    //max_count_x = max * 80;
    OCR1A = speed;
    if (direction != DIR_STOP) move_on_x = 1;
    else move_on_x = 0;
  ...
  if (step_toggle_x == 0) {
    if (move_on_x == 1)digitalWrite_D(X_STEP_PIN, HIGH);
    step_toggle_x = 1;
  }
  else {
    if (move_on_x == 1)digitalWrite_D(X_STEP_PIN, LOW);
    step_toggle_x = 0;
    ...
    limit_switch_x = digitalRead_C(X_STOP_PIN);
    if (limit_switch_x == 1)
    {
      if(reset_x == 0){
        TIMSK1 = 0x00;
  ...
  if (step_toggle_y == 0) {
    if (move_on_y == 1)digitalWrite_C(Y_STEP_PIN, HIGH);
    step_toggle_y = 1;
  }
  else {
    if (move_on_y == 1)digitalWrite_C(Y_STEP_PIN, LOW);
    step_toggle_y = 0;
    ...
    limit_switch_y = digitalRead_C(Y_STOP_PIN);
    if (limit_switch_y == 1)
    {
      if(reset_y == 0){
        TIMSK1 = 0x00;
...
#define digitalRead_A(PIN) ((PINA & PIN) != 0 ? 1 : 0)
#define digitalRead_B(PIN) ((PINB & PIN) != 0 ? 1 : 0)
#define digitalRead_C(PIN) ((PINC & PIN) != 0 ? 1 : 0)
#define digitalRead_D(PIN) ((PIND & PIN) != 0 ? 1 : 0)
...
double save_z_float_step = 0;

volatile int YZ_toggle = 0;
volatile int isDead = 0;

volatile int x_reset = 0;
volatile int x_step_toggle = 0;
volatile int x_step_count = 0;
volatile int x_step_count_max = 0;


volatile int y_reset = 0;
volatile int y_step_toggle = 0;
volatile int y_step_count = 0;
volatile int y_step_count_max = 0;


volatile int z_reset = 0;
volatile int z_step_toggle = 0;
volatile int z_step_count = 0;
volatile int z_step_count_max = 0;
...
void Z_move(int max_step, int dir, int step_speed)
{
  if (dir == DIR_UP) {
    ...
    digitalWrite_B(Z_DIR_PIN, LOW);
  }
  z_step_count_max = max_step;
  OCR3A = step_speed;
  YZ_toggle = 1;
  TIMSK3 = 0x02;
}
...
ISR(TIMER1_COMPA_vect) {
  if (x_step_toggle == 0) {
    digitalWrite_D(X_STEP_PIN, HIGH);
    x_step_toggle = 1;
  }
  else {
...
ISR(TIMER3_COMPA_vect) {
  if (YZ_toggle == 1) {
    if (z_step_toggle == 0) {
      digitalWrite_B(Z_STEP_PIN, HIGH);
      z_step_toggle = 1;
    }
    else {
      digitalWrite_B(Z_STEP_PIN, LOW);
      z_step_toggle = 0;
      z_step_count++;
    ...
    if (y_step_toggle == 0) {
      digitalWrite_C(Y_STEP_PIN, HIGH);
      y_step_toggle = 1;
    }
    else {
      digitalWrite_C(Y_STEP_PIN, LOW);
      y_step_toggle = 0;
      y_step_count++;
...
ISR(TIMER2_COMPA_vect)
{
  if (e_step_toggle == 0) {
    e_step_toggle = 1;
    digitalWrite_D(E_STEP_PIN, HIGH);
  }
  else {
    e_step_toggle = 0;
